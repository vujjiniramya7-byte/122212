...
const BACKEND_URL = (window.__NNRG_CONFIG__?.backendUrl) || import.meta.env.VITE_BACKEND_URL || "http://127.0.0.1:8000";
...
export default function ChatWidget() {
  ...
  const send = async (msg) => {
    ...
    try {
      ...
      setHistory((h) => [...h, {
        role: "bot",
        text: "❌ Cannot connect to the server right now.\n\nPlease try again in a moment. If the issue persists, contact NNRG at **admin@nnrg.edu.in** or **+91 8886531118**.",
      }]);
    } finally {
      setLoading(false);
    }
  ...
  const uploadFile = async (e) => {
    ...
    try {
      ...
      setHistory((h) => [...h, {
        role: "bot",
        text: "❌ Upload failed — could not reach the server.\n\nPlease try again in a moment.",
      }]);
    } finally {
      setUploading(false);
      e.target.value = "";
