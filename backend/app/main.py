...
- No local ML model loaded — embeddings via Gemini REST API directly
...
def _gemini_embed_batch(texts: list[str], task_type: str) -> list[list[float]]:
    """Call Gemini embedContent REST endpoint for each text."""
    ...
    for t in texts:
        ...
        if resp.status_code != 200:
            raise HTTPException(500, f"Gemini embed error {resp.status_code}: {resp.text[:300]}")
...
def embed_texts(texts: list[str]) -> np.ndarray:
    ...
    for i in range(0, len(texts), EMBED_BATCH):
        ...
        vecs = _gemini_embed_batch(batch, "RETRIEVAL_DOCUMENT")
...
def embed_query(text: str) -> np.ndarray:
    ...
    vecs = _gemini_embed_batch([text], "RETRIEVAL_QUERY")
...
async def seed_knowledge():
    ...
    if not any(d.get("doc_id") == "nnrg_builtin" for d in docs) and KNOWLEDGE_FILE.exists():
        try:
            ...
            print(f"WARNING: Could not index built-in knowledge at startup: {e}")
            print("The server will still start. Check GEMINI_API_KEY env var.")
